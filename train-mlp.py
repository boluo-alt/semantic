...
def main(args):
    ...
    train_dataset = GraphInterface(mode='train')
    valid_dataset = GraphInterface(mode='valid')
    test_dataset = GraphInterface(mode='test')
    exp_dataset = GraphInterface(mode='exp')
    ...
    exp_dataloader = DataLoader(exp_dataset, batch_size=1, collate_fn=collate, shuffle=False)
    ...
    for epoch in range(args.epochs):
        ...
        print("Epoch {:05d} | Time(s) {:.4f} | Loss: {:.4f}| Tran_acc: {:.4f}".format(epoch + 1,
                                                                                      np.mean(dur),
                                                                                      loss_data,
                                                                                      train_acc))
        val_loss_list = []
        ...
        for batch, subgraph in enumerate(valid_dataloader):
            with torch.no_grad():
                ...
                val_loss = loss_fcn(logits_val, subgraph.ndata['label'])
                val_acc = accuracy1(logits_val, subgraph.ndata['label'])
                val_loss_list.append(val_loss.item())
                val_acc_list.append(val_acc.item())

        val_loss_data = np.array(val_loss_list).mean()
        mean_val_loss = np.array(val_loss_list).mean()
        val_acc_data = np.array(val_acc_list).mean()
        mean_val_acc = np.array(val_acc_list).mean()
        print("Val_acc: {:.4f} | Loss: {:.4f}| ".format(mean_val_acc, val_loss_data))
        # early stop
        if mean_val_loss > best_score or best_loss > mean_val_loss:
            if best_score < mean_val_loss < best_loss:
    ...
    for batch, subgraph in enumerate(test_dataloader):
        ...
        test_acc = accuracy1(logits_test, subgraph.ndata['label'])
        test_loss = loss_fcn(logits_test, subgraph.ndata['label'])
        test_acc_list.append(test_acc.item())
        test_loss_list.append(test_loss.item())
    test_acc_data = np.array(test_acc_list).mean()
    test_loss_data = np.array(test_loss_list).mean()
    All_time.append(time.time() - time_all)
    print("Test Score: {:.4f}| Time(s) {:.4f}| Loss {:.4f}".format(test_acc_data,
                                                                   np.mean(All_time),
                                                                   test_loss_data))
